...
struct PaymentsView: View {
    ...
    @State private var isLoadingRecipientsAssets:Bool = false
    @State private var pathPaymentRecepientErrMsg:String?
    ...
    private var pathPaymentsView: some View  {
        GroupBox ("Send path payment"){
            ...
            recipientSelectionPicker
            if selectedRecipient != PaymentsView.selectRecipient {
                if let error = pathPaymentRecepientErrMsg {
                    Text("\(error)").font(.footnote).foregroundStyle(.red).frame(maxWidth: .infinity, alignment: .center)
                }
                if selectedRecipient == PaymentsView.otherRecipient {
                    HStack {
                        recipientAccountIdInputField
                        if isLoadingRecipientsAssets {
                            Utils.progressView
                        } else {
                            Button("Check", action:   {
                                Task {
    ...
    private func selectedRecepientChanged(oldValue:String, newValue:String) async {
        if pathPaymentMode && oldValue != newValue {
            print("changed from \(oldValue) to \(newValue)")
            if newValue != PaymentsView.otherRecipient && newValue != PaymentsView.selectRecipient {
                guard let contact = userContacts.filter({$0.id == newValue}).first else {
                    pathPaymentRecepientErrMsg = "Could not find recipient"
                    return
                }
    ...
    private func loadPathPaymentDestinationAssets(destAccountId:String) async {
        isLoadingRecipientsAssets = true
        if destAccountId.isEmpty {
            pathPaymentRecepientErrMsg = "Please insert the recipient`s account id"
            isLoadingRecipientsAssets = false
            return
        }
        if !destAccountId.isValidEd25519PublicKey() {
            pathPaymentRecepientErrMsg = "The recipient`s account id is not valid"
            isLoadingRecipientsAssets = false
            return
        }
        ...
        if let error =  recipientAssetsFetcher.error {
            ...
            case .accountNotFound(_):
                pathPaymentRecepientErrMsg = "The recipient account was not found on the Stellar Network. It needs to be funded first."
            case .fetchingError(_, let message):
                pathPaymentRecepientErrMsg = message
            }
            pathPaymentRecepientErrMsg = error.localizedDescription
        }
